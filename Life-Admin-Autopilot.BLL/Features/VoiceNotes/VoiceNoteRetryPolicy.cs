...
///
/// <para>
/// This classifier is why an unconfigured server takes ~20 s to settle a note
...
///
/// <para>
/// Identical numbers to <c>DocumentScanRetryPolicy</c> except the lock: 120 s here
/// against 180 s there, because Node sizes each lock to its own worst case and a
/// single-pass voice extraction is shorter than a multi-page vision call. Kept as
/// a separate policy rather than shared for exactly that reason.
/// </para>
/// </summary>
public static class VoiceNoteRetryPolicy
{
    private const int BackoffBaseMs = 2_000;
    private const int BackoffCapMs = 60_000;

    /// <summary>Node: <c>LOCK_MS</c> — "&gt; worst-case transcribe+extract for a 20-intent note".</summary>
    public static readonly TimeSpan Lock = TimeSpan.FromMilliseconds(120_000);
